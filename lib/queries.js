...
const queries = [
    ...
    `INSERT INTO employee (first_name, last_name, role_id, manager_id) VALUES (?, ?, ?, ?)`, // add an employee
    ``, // update an employee TODO
    `SELECT name FROM department`, // get departments
    `SELECT title FROM role`, // get roles
    `SELECT CONCAT(first_name, ' ', last_name) AS name FROM employee` // get employees
];
